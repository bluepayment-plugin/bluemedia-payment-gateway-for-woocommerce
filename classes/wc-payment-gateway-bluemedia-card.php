...
final class WC_Payment_Gateway_BlueMedia_Card extends WC_Payment_Gateway
{
    const ID_PAYMENT_GATEWAY_BLUEMEDIA_CARD = 'bluemedia_payment_gateway_card';

	protected $paymentGatewayBlueMedia;
	
	public function __construct() 
	{
		$this->id = self::ID_PAYMENT_GATEWAY_BLUEMEDIA_CARD;
		$this->paymentGatewayBlueMedia = new WC_Payment_Gateway_BlueMedia();

		$this->title = __("Card payment", 'bluepayment-gateway-for-woocommerce');
		$this->method_title = __("Blue Media online payment system", 'bluepayment-gateway-for-woocommerce');
        $this->method_description = __("Accept card payment", 'bluepayment-gateway-for-woocommerce');
	}
	...
	{
		$image_path = $this->paymentGatewayBlueMedia->settings['backgorund_channels'][Utils::get_current_currency()][Gateway::GATEWAY_ID_CARD]['iconURL'];
        $icon = sprintf(
            '<img src="%s" alt="%s" />',
            $image_path,
